		...
		case XFSBL_STAGE3:
			{
				...
				if (XFSBL_SUCCESS != FsblStatus)
				{
						...
						(FsblInstance.ImageHeader.ImageHeaderTable.NoOfPartitions-1U))
					{
						...
						/*
						 * Secure Boot Customization: Hash partitions and generate keygen seed
						 * Ported from Ultra96 FSBL (Vitis 2019.2)
						 *
						 * This code computes a SHA3-384 hash of multiple partitions and
						 * generates a keygen seed by combining the hash with the embedded
						 * development keys. The results are written to OCM for use by
						 * later boot stages.
						 *
						 * NOTE: Partition indices (4, 5, 6) are specific to the boot image
						 * configuration. Adjust these based on your actual boot.bin layout.
						 */
						...
						/* Partition in ATCM (partition index 4) */
						...
						// XFsbl_Printf(DEBUG_GENERAL, "Going into Sha3Init\r\n");
						XSecure_Sha3Initialize(&csu_sha3, &CsuDma);
						XSecure_Sha3Start(&csu_sha3);
						// XFsbl_Printf(DEBUG_GENERAL, "Going into Sha3Update\r\n");
						XSecure_Sha3Update(&csu_sha3, (u8*)kernel_load_addr, kernel_length);
						// XFsbl_Printf(DEBUG_GENERAL, "Leaving Sha3Update\r\n");

						/* Partition in OCM (partition index 5) */
						...
						/* Partition in DDR (partition index 6) */
						...
						/* Truncate the SHA3-384 output to SHA3-256 (32 bytes) */
						for (i = 0; i < 32; i++){
							XFsbl_Printf(DEBUG_GENERAL, "%02x", kernel_hash[i]);
