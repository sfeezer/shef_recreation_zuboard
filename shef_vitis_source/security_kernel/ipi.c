...
static XScuGic_Config *rpu_gic_lookup_config(void)
{
#ifdef SDT
	return XScuGic_LookupConfig(XPAR_XSCUGIC_0_BASEADDR);
#else
	return XScuGic_LookupConfigBaseAddr(XPAR_XSCUGIC_0_BASEADDR);
#endif
}

static XIpiPsu_Config *rpu_ipi_lookup_config(void)
{
#ifdef SDT
	return XIpiPsu_LookupConfig(XPAR_XIPIPSU_0_BASEADDR);
#else
#ifndef XPAR_XIPIPSU_0_DEVICE_ID
#error "XPAR_XIPIPSU_0_DEVICE_ID is missing from xparameters.h. Regenerate the BSP."
#endif
	return XIpiPsu_LookupConfig(XPAR_XIPIPSU_0_DEVICE_ID);
#endif
}

/**
* Initializes the RPU GIC (without connecting IPI interrupt yet)
*/
u32 rpu_gic_init(XScuGic *intc_inst_ptr, u32 int_id, Xil_ExceptionHandler handler,
	...
	intc_config = rpu_gic_lookup_config();
...
/**
* Initializes the RPU IPI, enables IPI Interrupts, and connects the GIC handler.
* NOTE: This must be called AFTER rpu_gic_init() so we can use ipi_inst.Config.IntId
*/
u32 rpu_ipi_init(XIpiPsu *ipi_inst_ptr){
	...
	/*
	 * In Vitis 2023.2 with SDT, Config.IntId contains the raw device tree
	 * interrupt specifier (e.g., 0x4021), not the actual GIC interrupt ID.
	 * Convert it using XGet_IntrId (extracts SPI number) + XGet_IntrOffset (adds 32 for SPI).
	 * For IPI1 (R5-0): 0x4021 -> SPI 33 -> GIC ID 65
	 */
	u32 raw_int_id = ipi_inst_ptr->Config.IntId;
	u32 gic_int_id = XGet_IntrId(raw_int_id) + XGet_IntrOffset(raw_int_id);
	// xil_printf("RPU: IPI raw IntId: 0x%x, GIC IntId: %d\r\n", raw_int_id, gic_int_id);

	//Connect the IPI interrupt handler to GIC using the converted GIC interrupt ID
	(void)XScuGic_Connect(&gic_inst, gic_int_id,
			(Xil_InterruptHandler)rpu_ipi_handler, (void *)ipi_inst_ptr);
	XScuGic_Enable(&gic_inst, gic_int_id);
...
* Signature matches Xil_InterruptHandler (void* parameter)
*/
void rpu_ipi_handler(void *callback_ref){
	XIpiPsu *ipi_inst_ptr = (XIpiPsu *)callback_ref;
	u32 reg_val;
	u32 status = XST_FAILURE;
	u32 ipi_buf[8];
	u32 resp_buf[2] = {PMU_IPI_HEADER, XST_SUCCESS};

	// Check if the IPI is from PMU channel
	reg_val = Xil_In32(IPI_CH1_ISR);
	// xil_printf("RPU ISR: %x\r\n", reg_val);
	//Checking against 0x10000
	if((reg_val & (u32)IPI_PMU_PM_INT_MASK_RECV) == 0U){
		// xil_printf("RPU: Error: Received IPI from invalid source, ISR:%x expected mask:%x \r\n", reg_val, IPI_PMU_PM_INT_MASK_RECV);
		return;
	}
