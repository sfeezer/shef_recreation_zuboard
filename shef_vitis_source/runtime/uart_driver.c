...
u32 handle_uart_cmd(void)
{
	...
	while (1) {
		...
		case UART_CMD_PUSH_USER_PK:
			...
			if (read_len != UART_PACKET_HEADER_SIZE + VERIFIER_PK_SIZE ||
			    user_pk_received == 1) {
				send_nack();
			} else {
				/* Copy the verifier's PK into memory */
				unsigned char *verif_pk_ptr =
				    (unsigned char *)(SHARED_MEM_BASE + VERIFIER_PK_OFFSET);
				memcpy(verif_pk_ptr, &read_buf[UART_PACKET_HEADER_SIZE],
				       VERIFIER_PK_SIZE);
				send_ack();
				user_pk_received = 1;
		...
		case UART_CMD_PUSH_USER_NONCE:
			if (read_len != UART_PACKET_HEADER_SIZE + NONCE_SIZE ||
			    user_nonce_received == 1) {
				send_nack();
			} else {
				/* Copy nonce into memory */
				unsigned char *verif_nonce_ptr =
				    (unsigned char *)(SHARED_MEM_BASE + NONCE_OFFSET);
				memcpy(verif_nonce_ptr, &read_buf[UART_PACKET_HEADER_SIZE],
				       NONCE_SIZE);
				send_ack();
				user_nonce_received = 1;
		...
		case UART_CMD_PULL_ATTESTATION:
			{
				/* pointer to attestation */
				unsigned char *attestation_ptr =
				    (unsigned char *)(SHARED_MEM_BASE + ATTESTATION_OFFSET);
				u32 bytes_sent = 0;
				...
				while (bytes_sent < ATTESTATION_SIZE + SHARED_SECRET_SIG_SIZE) {
					...
					if (bytes_sent + 188 >
					    ATTESTATION_SIZE + SHARED_SECRET_SIG_SIZE) {
						bytes_to_send =
						    (ATTESTATION_SIZE + SHARED_SECRET_SIG_SIZE) -
						    bytes_sent;
					}
					...
					write_packet(&attestation_ptr[bytes_sent],
						     ...
						     ((bytes_sent / 4) & 0xff), bytes_to_send);
		...
		case UART_CMD_PUSH_BITSTREAM_KEY:
			if (read_len != UART_PACKET_HEADER_SIZE + BITSTREAM_KEY_SIZE) {
				...
				unsigned char *decryption_key_ptr =
				    (unsigned char *)(SHARED_MEM_BASE + BITSTREAM_KEY_OFFSET);
				memcpy(decryption_key_ptr, &read_buf[UART_PACKET_HEADER_SIZE],
				       BITSTREAM_KEY_SIZE);
				send_ack();
		...
		case UART_CMD_PUSH_FPGA_DATA:
			...
			user_data = read_buf[3] << 24 | read_buf[4] << 16 | read_buf[5] << 8 |
				    read_buf[6];
			accel[1] = read_buf[3] << 24 | read_buf[4] << 16 | read_buf[5] << 8 |
				   read_buf[6];
			accel[2] = read_buf[7] << 24 | read_buf[8] << 16 | read_buf[9] << 8 |
				   read_buf[10];
			accel[3] = read_buf[11] << 24 | read_buf[12] << 16 | read_buf[13] << 8 |
				   read_buf[14];
			accel[4] = read_buf[15] << 24 | read_buf[16] << 16 | read_buf[17] << 8 |
				   read_buf[18];
